...
const EditableFieldGeneralWithAutoComplete = (props) => {
  ...
  const [autoCompleteDataBackup, setAutoCompleteDataBackup] = useState([]) 
  ...
  const getAutoCompleteData = props.getAutoCompleteData || ((value)=>{console.log(`Trying to get autocomplete data for ${value}`); return []})
  ...
  var handleAutocompleteKeyDown = (evt) => {
    ...
    if(evt.keyCode === 38){
      ...
      // Query Data store to get auto complete data for the current text entry
      // If store is not available, use backup data
      let _autoCompleteDataBackup = getAutoCompleteData(_content.toLocaleLowerCase())
      console.log(_autoCompleteDataBackup)
      if((_autoCompleteDataBackup.length > 0) && (JSON.stringify(_autoCompleteDataBackup) !== JSON.stringify(autoCompleteDataBackup))){
        setAutoCompleteDataBackup(autoCompleteDataBackup => _autoCompleteDataBackup)
      }else{
        _autoCompleteDataBackup = autoCompleteDataBackup
      }


      // Using content's updated shadow copy, filter the data array to only display the relevant entries
      let _displayedData = _autoCompleteDataBackup.filter(item => item.substr(0, _content.length).toUpperCase() === _content.toUpperCase())
